...
/obj/item/device/motiondetector/proc/Redraw(mob/user)
	while(active && user.client)
		...
		for(var/mob/living/t in range(7,M))
			if(t != M)
		...
		for(var/mob/living/t in range(7,M))
			if((t.tracker_position != null) && (get_dist(t.tracker_position, t.loc) >= 1))
				t.tracker_position = null
				detected += t

		if(detected.len>=1)
			for(var/mob/living/t in detected)
				var/obj/Blip/o = unpool("detector_blips", /obj/Blip) // Get a blip from the blip pool
				o.pixel_x = (t.x-M.x)*8-4 // Make the blip in the right position on the radar (multiplied by the icon dimensions)
				o.pixel_y = (t.y-M.y)*8-4 //-4 is a slight offset south and west
